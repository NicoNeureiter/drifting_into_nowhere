...
if __name__ == '__main__':
    if not os.path.exists('data/migration'):
        os.mkdir('data/migration')
    if not os.path.exists('results'):
        os.mkdir('results')
    if not os.path.exists('results/migration'):
        os.mkdir('results/migration')

    logging.basicConfig(filename='results/migration/nowhere.log', level=logging.DEBUG)
    logging.getLogger().addHandler(logging.StreamHandler())

    logging.info('\n################################  NEW RUN  ################################')

    SCRIPT_PATH = 'src/beast_pipeline.sh'
    XML_PATH = 'data/migration/nowhere.xml'
    TREE_PATH = 'data/migration/nowhere.tree'
    ...
    for i_run in range(N_RUNS):
        ...
        os.system('bash {script} {hpd} {burnin} {cwd}'.format(
            script=SCRIPT_PATH, hpd=HPD, burnin=BURNIN, cwd='data/migration/'))
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig('results/migration/run_%i.pdf' % i_run, format='pdf')
